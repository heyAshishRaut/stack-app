...
function Tags() {
    ...
    return (
        <div className="w-full min-h-screen pb-[100px] flex flex-col px-[30px] relative overflow-hidden">
            <AnimatePresence>
                {eachLink && (
                    <motion.div
            ...
            <div className="mt-[50px] mb-[50px] h-[70px] flex gap-x-[30px] items-center">
                ...
                <div className="text-white font-cabinet text-5xl tracking-wide">Tags</div>
            </div>

            <div className="flex gap-x-[15px] w-[100%] text-sm">
                {allTags.map(({ _id, tagname, color }) => (
                    <button
                        type="button"
                        key={_id}
                        onClick={() => toggleTag(_id)}
                        className={`bg-gray-500/30 px-[30px] py-[10px] rounded-[7px] cursor-pointer flex items-center justify-center text-white
                            `}
                        style={{
                            backgroundColor: tagforlink.includes(_id) ? `${color}90` : ``
                        }}>
                        {tagname}
                    </button>
                ))}
            ...
            <div className="flex mt-[50px] gap-x-[22px] gap-y-[22px] mb-[100px] w-[100%] flex-wrap">
                {data && data.message && data.message.length > 0 ?
                    (data.message.map((item: any) => {
                        if (
                            tagforlink.length > 0 &&
                            !tagforlink.every(id => item.tags.some((tag: any) => tag._id === id))
                        ) {
                            return null;
                        }
